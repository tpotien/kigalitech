...
const SUPPORT_EMAIL = 'kigalitechservices@gmail.com';
...
async function sendViaResend(to, subject, html) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.EMAIL_FROM || `${BRAND} <hello@kigalitechservices.com>`,
      reply_to: SUPPORT_EMAIL,
      to,
      subject,
      html,
      headers: {
        'X-Entity-Ref-ID': `kigalitech-${Date.now()}`,
  ...
  if (!res.ok) {
    ...
    return false; // fall through to Gmail
  }
  return true;
}
...
async function send(to, subject, html, text) {
  ...
  if (gmail) {
    try {
      await gmail.sendMail({ from: FROM, to, subject, html, text: plainText });
      console.log(`[email] Sent via Gmail to ${recipient}`);
      return;
    } catch (err) {
  ...
  if (smtp) {
    try {
      await smtp.sendMail({ from: FROM, to, subject, html, text: plainText });
  ...
  console.error(`[email] ALL transports failed — email NOT delivered | to=${recipient} | subject=${subject}`);
}
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
  ...
  <tr><td style="background:#f8fafc;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 16px 16px;padding:24px 40px;text-align:center">
    <p style="margin:0 0 4px;font-size:13px;color:#64748b">📧 <a href="mailto:${SUPPORT_EMAIL}" style="color:#0ea5e9;text-decoration:none">${SUPPORT_EMAIL}</a></p>
    <p style="margin:0 0 8px;font-size:13px;color:#64748b">📞 <a href="tel:${SUPPORT_PHONE.replace(/\s/g,'')}" style="color:#0ea5e9;text-decoration:none">${SUPPORT_PHONE}</a> · <a href="https://wa.me/${WHATSAPP}" style="color:#25d366">WhatsApp</a></p>
    <p style="margin:0;font-size:12px;color:#94a3b8">© ${new Date().getFullYear()} ${BRAND} · KN 74St, Kigali, Rwanda</p>
    <p style="margin:8px 0 0;font-size:11px;color:#cbd5e1">Questions? Reply to this email or WhatsApp us at ${SUPPORT_PHONE}.</p>
  </td></tr>
</table>
...
function itemsTable(items) {
  const rows = items.map(i => `
    <tr>
      ...
      <td style="padding:10px 8px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:700;font-size:14px">${rwf(i.price * i.quantity)}</td>
    </tr>`).join('');
...
export async function sendOrderConfirmation({ order, shippingName, shippingEmail, items }) {
  ...
  const body = `
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
      <tr><td style="padding:6px 0;font-size:14px;color:#64748b">Subtotal</td><td style="padding:6px 0;font-size:14px;text-align:right">${rwf(order.total - (order.discountAmount || 0))}</td></tr>
      ${order.discountAmount ? `<tr><td style="padding:6px 0;font-size:14px;color:#10b981">Discount</td><td style="padding:6px 0;font-size:14px;text-align:right;color:#10b981">−${rwf(order.discountAmount)}</td></tr>` : ''}
      <tr style="border-top:2px solid #f1f5f9"><td style="padding:10px 0 0;font-size:16px;font-weight:800">Total Paid</td><td style="padding:10px 0 0;font-size:16px;font-weight:800;text-align:right">${rwf(order.total)}</td></tr>
    </table>
...
export async function sendVerificationEmail({ email, name, code }) {
  ...
  const body = `
    ...
    <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:12px;padding:16px;margin:0 0 24px;font-size:13px;color:#0369a1">
      <strong>🔒 Security tip:</strong> Only enter this code on the KigaliTech website. Do not share it with anyone who calls you.
    </div>
...
export async function sendMagicLinkEmail({ email, name, token, isNew }) {
  if (!email || email.includes('@phone.kigalitech.com')) return;
  const link = `${SITE_URL}/api/auth/magic-verify?token=${token}&email=${encodeURIComponent(email)}`;
  const body = `
    <p style="font-size:18px;margin:0 0 8px">Hi ${name || 'there'} 👋</p>
    <p style="color:#64748b;margin:0 0 28px">${isNew ? `Welcome to ${BRAND}! Click the button below to verify your email and access your account.` : `Here is your one-tap access button for your ${BRAND} account.`}</p>
    <div style="text-align:center;margin:32px 0">
      <a href="${link}" style="background:#0ea5e9;color:#fff;padding:16px 40px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block;">
        ${isNew ? 'Verify &amp; Access Account' : 'Access My Account'}
      </a>
    </div>
    <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:12px;padding:16px;margin:0 0 24px;font-size:13px;color:#0369a1">
      <strong>⏱ This button expires in 15 minutes</strong> and works only once.<br/>
      <span style="color:#64748b">If you did not request this, no action is needed — your account is safe.</span>
    </div>
    <p style="font-size:12px;color:#94a3b8;text-align:center">Or copy this link into your browser address bar:</p>
    <p style="font-size:11px;color:#0ea5e9;text-align:center;word-break:break-all;margin-top:4px">${link}</p>`;
  const textBody = `Hi ${name || 'there'},\n\nHere is your KigaliTech account access link:\n\n${link}\n\nValid for 15 minutes, single use only.\nIf you did not request this, ignore this email — your account is safe.\n\n— KigaliTech Team\n${SUPPORT_EMAIL}`;
  await send(email, `${name ? name + ' — your' : 'Your'} KigaliTech account access`, layout('Account Access', '#0ea5e9', body), textBody);
}
...
export async function broadcastNewProduct({ product, subscribers }) {
  const key = process.env.RESEND_API_KEY;
  if (!key || key === 'placeholder' || !subscribers?.length) return;

  const productUrl = `${SITE_URL}/products/${product.id}`;
  const price = product.price ? `RWF ${Number(product.price).toLocaleString()}` : '';
  const comparePrice = product.comparePrice && product.comparePrice > product.price
    ? `RWF ${Number(product.comparePrice).toLocaleString()}` : null;
  const discount = comparePrice
    ? Math.round((1 - product.price / product.comparePrice) * 100) : 0;

  function buildHtml(name) {
    const body = `
      <p style="font-size:18px;margin:0 0 6px">Hi ${name || 'there'} 👋</p>
      <p style="color:#64748b;margin:0 0 24px">We just added something new to our store — and we thought you'd want to be the first to know.</p>

      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:16px;padding:24px;margin:0 0 24px;text-align:center">
        ${product.category ? `<div style="font-size:11px;font-weight:700;color:#0ea5e9;letter-spacing:2px;text-transform:uppercase;margin-bottom:10px">${product.category}</div>` : ''}
        <div style="font-size:22px;font-weight:800;color:#0f172a;margin-bottom:12px;line-height:1.3">${product.name}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap">
          <span style="font-size:26px;font-weight:900;color:#0f172a">${price}</span>
          ${comparePrice ? `<span style="font-size:14px;color:#94a3b8;text-decoration:line-through">${comparePrice}</span>` : ''}
          ${discount > 0 ? `<span style="background:#fee2e2;color:#ef4444;font-size:12px;font-weight:700;padding:3px 10px;border-radius:99px">-${discount}%</span>` : ''}
        </div>
        ${product.brand ? `<div style="font-size:12px;color:#94a3b8;margin-top:8px">${product.brand}</div>` : ''}
      </div>

      ${product.description ? `<p style="color:#64748b;font-size:14px;margin:0 0 24px;line-height:1.6">${product.description.slice(0, 160)}${product.description.length > 160 ? '…' : ''}</p>` : ''}

      ${btn(productUrl, 'View Product →', '#0ea5e9')}

      <p style="margin:24px 0 0;font-size:11px;color:#94a3b8;text-align:center">
        You're receiving this because you subscribed to KigaliTech updates.<br/>
        <a href="${SITE_URL}/unsubscribe?email={{email}}" style="color:#cbd5e1">Unsubscribe</a>
      </p>`;
    return layout(`New: ${product.name}`, '#0ea5e9', body);
  }
  ...
  for (let i = 0; i < subscribers.length; i += BATCH_SIZE) {
    const batch = subscribers.slice(i, i + BATCH_SIZE).map(sub => ({
      ...
      reply_to: SUPPORT_EMAIL,
      to: sub.email,
      subject: `🆕 Just In: ${product.name} — ${price}`,
      html: buildHtml(sub.name).replace('{{email}}', encodeURIComponent(sub.email)),
    }));
    ...
    }).catch(err => console.error('[email] broadcast batch failed:', err.message));
  }
}
...
export async function sendContactFormEmail({ name, email: senderEmail, subject, message }) {
  const body = `
    <p style="font-size:16px;font-weight:700;margin:0 0 4px">New contact form submission</p>
    <p style="color:#64748b;margin:0 0 20px;font-size:13px">From the KigaliTech website contact form</p>
    <table width="100%" style="border-collapse:collapse;margin:0 0 20px">
      <tr><td style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:13px;color:#64748b;width:80px">Name</td><td style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:14px;font-weight:600">${name}</td></tr>
      <tr><td style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:13px;color:#64748b">Email</td><td style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:14px"><a href="mailto:${senderEmail}" style="color:#0ea5e9">${senderEmail}</a></td></tr>
      <tr><td style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:13px;color:#64748b">Subject</td><td style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:14px">${subject}</td></tr>
    </table>
    <div style="background:#f8fafc;border-left:4px solid #0ea5e9;border-radius:0 12px 12px 0;padding:16px;margin:0 0 20px">
      <p style="font-size:13px;color:#64748b;margin:0 0 8px;font-weight:600">Message:</p>
      <p style="font-size:14px;margin:0;white-space:pre-wrap">${message.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</p>
    </div>
    <p style="font-size:12px;color:#94a3b8">Reply directly to this email to respond to the customer.</p>`;
  const recipients = ['kigalitechservices@gmail.com', 'tpotien1@gmail.com'];
  await send(recipients, `📬 Contact: ${subject} — from ${name}`, layout('Contact Form', '#0ea5e9', body));
}

export async function sendReviewRequestEmail({ email, name, orderId, items }) {
  if (!email) return;
  const firstName = (name || 'there').split(' ')[0];
  const item = items?.[0];
  const subject = `How was your order, ${firstName}? Leave a quick review ⭐`;
  const itemRows = (items || []).slice(0, 3).map(i => {
    const imgs = (() => { try { return JSON.parse(i.product?.images || '[]'); } catch { return []; } })();
    const img = imgs[0] || '';
    return `
      <tr>
        <td style="padding:10px 8px;border-bottom:1px solid #f1f5f9;vertical-align:middle">
          ${img ? `<img src="${img}" width="48" height="48" style="border-radius:8px;object-fit:contain;vertical-align:middle;margin-right:10px" />` : ''}
          <span style="font-size:14px;font-weight:600">${i.name || ''}</span>
        </td>
        <td style="padding:10px 8px;border-bottom:1px solid #f1f5f9;text-align:right;vertical-align:middle">
          <a href="${SITE_URL}/products/${i.productId}" style="background:#0ea5e9;color:#fff;text-decoration:none;border-radius:20px;padding:6px 14px;font-size:12px;font-weight:700">Review</a>
        </td>
      </tr>`;
  }).join('');
  const body = `
    <p style="font-size:18px;margin:0 0 8px">Thanks for your order, ${firstName}! 🎉</p>
    <p style="color:#64748b;margin:0 0 24px">We hope you're loving your new tech. Share your experience — it helps other shoppers and takes less than a minute.</p>
    ...
    ${btn(`${SITE_URL}/orders/${orderId}`, 'See My Order', '#0ea5e9')}
    <p style="margin:16px 0 0;font-size:12px;color:#94a3b8">Questions? WhatsApp us: <a href="https://wa.me/250786276555" style="color:#0ea5e9">+250 786 276 555</a></p>`;
  await send(email, subject, layout('Rate Your Purchase', '#f59e0b', body));
}

// ─── Back-in-stock notification ──────────────────────────────────────────────

export async function sendBackInStockEmail({ email, productName, productId }) {
  if (!email) return;
  const body = `
    <p style="font-size:18px;margin:0 0 8px">Great news! 🎉</p>
    <p style="color:#64748b;margin:0 0 24px">The item you were waiting for is back in stock at ${BRAND}.</p>
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:20px;margin:0 0 24px;text-align:center">
      <div style="font-size:20px;font-weight:800;color:#0f172a;margin-bottom:8px">${productName}</div>
      <div style="font-size:13px;color:#16a34a">Now available — limited stock, order soon</div>
    </div>
    ${btn(`${SITE_URL}/products/${productId}`, 'Shop Now')}
    <p style="margin:20px 0 0;font-size:12px;color:#94a3b8">You signed up for stock alerts on ${BRAND}. <a href="${SITE_URL}/api/newsletter/unsubscribe?email=${encodeURIComponent(email)}" style="color:#94a3b8">Unsubscribe</a></p>`;
  await send(email, `✅ Back in stock: ${productName} — ${BRAND}`, layout('Back in Stock', '#10b981', body));
}

// ─── Price drop alert ────────────────────────────────────────────────────────

export async function sendPriceDropEmail({ email, productName, productId, oldPrice, newPrice }) {
  if (!email) return;
  const savePct = Math.round(((oldPrice - newPrice) / oldPrice) * 100);
  const body = `
    <p style="font-size:18px;margin:0 0 8px">Price drop alert! 🔥</p>
    <p style="color:#64748b;margin:0 0 24px">An item on your watchlist just got cheaper.</p>
    <div style="background:#fef9c3;border:1px solid #fde68a;border-radius:12px;padding:20px;margin:0 0 24px">
      <div style="font-size:18px;font-weight:800;color:#0f172a;margin-bottom:12px">${productName}</div>
      <div style="display:flex;gap:16px;align-items:center">
        <div>
          <div style="font-size:12px;color:#92400e;text-transform:uppercase;letter-spacing:.05em">Was</div>
          <div style="font-size:16px;color:#9ca3af;text-decoration:line-through">${rwf(oldPrice)}</div>
        </div>
        <div style="font-size:22px;color:#9ca3af">→</div>
        <div>
          <div style="font-size:12px;color:#92400e;text-transform:uppercase;letter-spacing:.05em">Now</div>
          <div style="font-size:22px;font-weight:800;color:#dc2626">${rwf(newPrice)}</div>
        </div>
        <div style="margin-left:auto;background:#dc2626;color:#fff;font-size:14px;font-weight:800;padding:6px 12px;border-radius:8px">-${savePct}%</div>
      </div>
    </div>
    ${btn(`${SITE_URL}/products/${productId}`, 'Grab This Deal', '#dc2626')}
    <p style="margin:20px 0 0;font-size:12px;color:#94a3b8">You set a price alert on ${BRAND}. <a href="${SITE_URL}/api/newsletter/unsubscribe?email=${encodeURIComponent(email)}" style="color:#94a3b8">Unsubscribe</a></p>`;
  await send(email, `🔥 Price drop: ${productName} now ${rwf(newPrice)} — ${BRAND}`, layout('Price Drop Alert', '#dc2626', body));
}
