...
async function sendViaResend(to, subject, html) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.EMAIL_FROM || `${BRAND} <noreply@kigalitechservices.com>`,
      reply_to: 'noreply@kigalitechservices.com',
      to,
      subject,
      html,
    }),
...
async function send(to, subject, html, text) {
  try {
    ...
    if (gmail) {
      await gmail.sendMail({ from: FROM, to, subject, html, text: text || html.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim() });
      console.log(`[email] Sent via Gmail to ${Array.isArray(to) ? to.join(', ') : to}`);
      return;
    }
    const smtp = getSmtpTransport();
    if (smtp) {
      await smtp.sendMail({ from: FROM, to, subject, html, text: text || html.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim() });
      return;
    }
    console.error('[email] No transport configured — email NOT sent to', to);
  } catch (err) {
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
  ...
  <tr><td style="background:#f8fafc;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 16px 16px;padding:24px 40px;text-align:center">
    ...
    <p style="margin:8px 0 0;font-size:11px;color:#cbd5e1">This is an automated email — please do not reply.</p>
  </td></tr>
</table>
...
export async function sendMagicLinkEmail({ email, name, token, isNew }) {
  ...
  const body = `
    <p style="font-size:18px;margin:0 0 8px">Hi ${name || 'there'} 👋</p>
    <p style="color:#64748b;margin:0 0 28px">${isNew ? `Welcome to ${BRAND}! Click the button below to verify your email and log in instantly.` : `Click the button below to sign in to your ${BRAND} account. No password needed.`}</p>
    <div style="text-align:center;margin:32px 0">
      <a href="${link}" style="background:#0ea5e9;color:#fff;padding:16px 40px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block;">
        ${isNew ? 'Verify &amp; Sign In' : 'Sign In to KigaliTech'}
      </a>
    </div>
    <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:12px;padding:16px;margin:0 0 24px;font-size:13px;color:#0369a1">
      <strong>⏱ This link expires in 15 minutes</strong> and can only be used once.<br/>
      <span style="color:#64748b">If you didn't request this, you can safely ignore this email.</span>
    </div>
    <p style="font-size:12px;color:#94a3b8;text-align:center;margin-top:16px">If you don't see this email, check your spam folder and mark it as "Not Spam".</p>
    <p style="font-size:12px;color:#94a3b8;text-align:center">Or copy this link: <span style="color:#0ea5e9">${link}</span></p>`;
  const textBody = `Hi ${name || 'there'},\n\nYou requested a sign-in link for KigaliTech. Click the link below:\n\n${link}\n\nThis link expires in 15 minutes and can only be used once. If you didn't request this, you can safely ignore this email.\n\n— KigaliTech Team\nkigalitechservices@gmail.com`;
  await send(email, `Your KigaliTech sign-in link`, layout('Sign-In Link', '#0ea5e9', body), textBody);
}
