...
async function send(to, subject, html) {
  try {
    ...
    if (gmail) {
      await gmail.sendMail({ from: FROM, to, subject, html });
...
export async function sendMagicLinkEmail({ email, name, token, isNew }) {
  ...
  const body = `
    ...
    <div style="text-align:center;margin:32px 0">
      <a href="${link}" style="display:inline-block;background:linear-gradient(135deg,#0ea5e9,#2563eb);color:#fff;padding:18px 48px;border-radius:50px;text-decoration:none;font-weight:800;font-size:16px;letter-spacing:-0.3px;box-shadow:0 8px 24px rgba(14,165,233,0.35)">
  ...
  await send(email, `🔑 Your ${BRAND} sign-in link`, layout('Magic Sign-In Link', '#0ea5e9', body));
}
