export default function WhatsAppButton({ phone, message }) {
  const number = (phone || process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '250700000000').replace(/\D/g, '');
  const text = encodeURIComponent(message || 'Hi KigaliTech! I need help with a product.');
  ...
  return (
    <a
      ...
      className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-lg shadow-green-400/30 hover:bg-[#20bf5b] hover:scale-110 transition-all"
