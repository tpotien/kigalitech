        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 ? (
            ...
            <ul className="space-y-4">
              {items.map((item) => (
                <li key={item.key} className="flex gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <img
                    src={item.image}
                    alt={item.name}
                  ...
                  <div className="flex flex-1 flex-col gap-1">
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      ...
                      <p className="font-bold text-slate-900">${((item.price * item.quantity) / 100).toFixed(2)}</p>
                    </div>
                  </div>
                </li>
              ))}
        ...
        {items.length > 0 && (
          <div className="border-t border-slate-100 px-6 py-5 space-y-4">
            <div className="flex items-center justify-between text-sm text-slate-500">
              <span>Subtotal</span>
              <span className="text-lg font-bold text-slate-900">${(subtotal / 100).toFixed(2)}</span>
            </div>
