export default function AvatarWithBadge({ image, name, role, emailVerified, size = 'md' }) {
  ...
  function Badge() {
    if (role === 'admin') return (
      <span className={`absolute -bottom-0.5 -right-0.5 ${badgeDim[size]} rounded-full bg-amber-400 border-2 border-white dark:border-slate-900 flex items-center justify-center shadow`} title="Admin">
    ...
    if (role === 'staff') return (
      <span className={`absolute -bottom-0.5 -right-0.5 ${badgeDim[size]} rounded-full bg-sky-500 border-2 border-white dark:border-slate-900 flex items-center justify-center shadow`} title="Staff">
    ...
    if (emailVerified) return (
      <span className={`absolute -bottom-0.5 -right-0.5 ${badgeDim[size]} rounded-full bg-emerald-500 border-2 border-white dark:border-slate-900 flex items-center justify-center shadow`} title="Verified">
  ...
  return (
    <span className={`relative inline-flex flex-shrink-0 ${isLarge ? '' : ''}`}>
      {image ? (
        ...
        <span className={`${dim[size]} flex items-center justify-center rounded-full bg-sky-600 ${textSize[size]} font-bold text-white`}>
          {initial}
        </span>
      )}
      <Badge />
