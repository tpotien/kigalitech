...
function RepairCard({ ticket, onUpdate }) {
  ...
  async function handleQuote(e) {
    e.preventDefault();
    const rwf = Math.round(parseFloat(quotedCost));
    if (!rwf || rwf <= 0) return;
    const ok = await patch({ action: 'set_quote', quotedCost: rwf, quoteNotes });
    ...
    <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="px-5 pt-4">
          ...
          <div className={`rounded-xl p-4 mb-3 ${
            ...
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div>
                ...
                <p className="text-2xl font-extrabold text-slate-900 mt-0.5">RWF {Math.round(ticket.quotedCost).toLocaleString()}</p>
        ...
        {!showQuoteForm && ticket.quoteStatus !== 'confirmed' && ticket.quoteStatus !== 'rejected' && (
          <button
            onClick={() => { setShowQuoteForm(true); setQuotedCost(ticket.quotedCost ? String(Math.round(ticket.quotedCost)) : ''); setQuoteNotes(ticket.quoteNotes || ''); }}
