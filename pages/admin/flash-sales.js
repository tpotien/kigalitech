              ...
              {selectedProduct ? (
                <div className="flex items-center gap-3 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500">Current price: RWF {Math.round(selectedProduct.price).toLocaleString()}</p>
                  </div>
                  <button
                    type="button"
                      ...
                      {searchResults.map(p => (
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-slate-400">RWF {Math.round(p.price).toLocaleString()} · {p.category}</p>
                          </div>
                          {p.flashSalePrice && (
            ...
            {selectedProduct && (
              <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-sm">
                ...
                <p className="text-amber-700 mt-0.5">
                  Original: <span className="line-through">RWF {Math.round(selectedProduct.price).toLocaleString()}</span>
                  {' → '}
                  Sale price: <span className="font-bold">RWF {Math.round(selectedProduct.price * (1 - discountPct / 100)).toLocaleString()}</span>
                  {' — '}Ends in {hoursLeft}h
                </p>
              </div>
            )}
                ...
                {products.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-500">RWF {Math.round(p.price).toLocaleString()}</td>
                      <td className="px-6 py-4 font-bold text-orange-600">RWF {Math.round(p.flashSalePrice).toLocaleString()}</td>
